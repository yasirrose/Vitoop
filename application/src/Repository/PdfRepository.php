...
class PdfRepository extends ResourceRepository
{
    ...
    public function getResourcesQuery(SearchResource $search)
    {
        $qb = $this->createQueryBuilder('r')
            ->select('ANY_VALUE(r.author) AS author, ANY_VALUE(r.url) AS url, ANY_VALUE(r.tnop) AS tnop, ANY_VALUE(r.isDownloaded) AS isDownloaded, ANY_VALUE(r.pdfDate.date) as pdfDate, ANY_VALUE(r.pdfDate.order) as HIDDEN orderDate')
            ->addGroupBy('r.author')
            ->addGroupBy('r.url')
            ->addGroupBy('r.tnop')
    ...
    {
        return <<<'EOT'
            SELECT SQL_CALC_FOUND_ROWS base.coef, base.coefId, base.text, base.author, base.url, base.tnop, base.isDownloaded, base.pdfDate, base.id, base.name, base.created_at, base.username, base.avgmark, base.res12count, base.isUserHook, base.isUserRead, base.color
              FROM (
               %s
               UNION ALL
