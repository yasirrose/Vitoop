...
class RatingRepository extends ServiceEntityRepository
{
    ...
    public function getRatingFromResourceByUser(Resource $resource, $user)
    {
        ...
        return $this
            ->getEntityManager()
            ->createQuery('SELECT r FROM App\Entity\Rating r WHERE r.resource=:arg_resource AND r.user=:arg_user ')
    ...
    public function getMarkFromResourceByUser(Resource $resource, $user)
    {
        ...
        $result = $this
            ->getEntityManager()
            ->createQuery('SELECT r.mark FROM App\Entity\Rating r WHERE r.resource=:arg_resource AND r.user=:arg_user ')
            ->setParameters(array('arg_resource' => $resource, 'arg_user' => $user))
            ->getOneOrNullResult();
    ...
    public function getAverageMarkFromResource(Resource $resource)
    {
        return $this
            ->getEntityManager()
            ->createQuery('SELECT AVG(r.mark) FROM App\Entity\Rating r WHERE r.resource=:arg_resource')
