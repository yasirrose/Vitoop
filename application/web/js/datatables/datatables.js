 ...
 *   https://datatables.net/download/#ju/dt-1.10.9,rr-1.0.0
 *
 * Included libraries:
 *   DataTables 1.10.9, RowReorder 1.0.0
 */
		...
		"fnStateLoadCallback": function ( settings ) {
			try {
				...
				var search = JSON.parse(
					(settings.iStateDuration === -1 ? sessionStorage : localStorage).getItem(
						'DataTables_search_'+location.pathname
					)
				);
				if (search != null) {
					test.search = search;
				...
				var pages = JSON.parse(
						(settings.iStateDuration === -1 ? sessionStorage : localStorage).getItem(
								'DataTables_pages_'+location.pathname
						)
				);
				if (pages != null) {
					test.length = pages;
...
jQuery.fn.dataTable.ext.builder = "ju\/dt-1.10.9,rr-1.0.0";
